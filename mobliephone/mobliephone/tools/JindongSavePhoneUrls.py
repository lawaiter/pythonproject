...
phone = get_xiaomi_phone_list() + get_meizu_phone_list() + get_huawei_phone_list()


# 进行京东手机商品类型对应的urls地址
def get_phones_format_urls(phone):
    format_urls = []
    phone_lists = phone
...
def create_database_and_tables():
    ...
    cursor = conn.cursor()
    # 创建京东数据库的语句
    create_database = """CREATE  DATABASE IF NOT EXISTS JingdongPhoneUrls"""
    # 创建手机和对应网店地址数据表
    ...
    try:
        ...
        conn.select_db("JingdongPhoneUrls")
        try:
            cursor.execute(create_table_phoneurls)
            print("phoneurls数据表创建成功")
            cursor.execute(create_table_phonecomments)
            cursor.commit()
        except:
            cursor.execute(create_table_phonecomments)
            print("phonecomments数据表创建成功")
            cursor.commit()
    finally:
        cursor.close()
        conn.close()
...
# 将某种类型的手机所有的网店地址存入数据表
def save_every_phone_url_list():
    global phone
    create_database_and_tables()
    insert_sql = "INSERT INTO phoneurls(phone_type, url) VALUES(%s, %s)"
    conn = MySQLdb.connect(host='127.0.0.1', user='root', passwd='tentan', db='JingdongPhoneUrls', charset="utf8", use_unicode=True)
    cursor = conn.cursor()
    format_urls = get_phones_format_urls(phone)
